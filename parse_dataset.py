...
# @todo: maybe it makes no sense to predict these at every time step in the VAE. They might instead be considered as actions
...
def add_log_actions(data):
    data['log_vaso'] = data.max_dose_vaso.apply(lambda x: np.log(x + 1e-5))
    data['log_fluid'] = data.input_4hourly_tev.apply(lambda x: np.log(x + 1e-3))


# @todo: maybe the action should be preprocessed with log or/and standard scaling
def matrify_histories(data, idx, scaler, log_scaler, action_scaler, T=25, verbose=True, log_action=True):
    ...
    return X_action_bin, X_features_bin, X_num, X_action, X_finished, X_alive


def transition_iterator(data, idx, scaler, log_scaler, action_scaler, RMAX=15, log_action=True):
    """
    Put the dataset in form of a list of transition (s,a,r,s')
    :param data: the pandas dataframe containing the sepsis dataset
    :param idx: the indexes of the patients you want to consider (for example for split into train and test set)
...
if __name__ == '__main__':
    ...
    X_action_bin, X_features_bin, X_num, X_action, X_finished, X_alive = matrify_histories(data, train_idx, scaler, log_scaler, action_scaler, verbose=False)
